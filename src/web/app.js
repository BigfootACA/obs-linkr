...
function t(key, params = {})
{
	...
	return template.replace(/\{([a-zA-Z0-9_]+)\}/g,
				(_, name) => { return params[name] === undefined ? "" : String(params[name]); });
}
...
function userMessage(error)
{
	...
	if (/Missing username/i.test(message))
		return t("missingUsername");
	if (/Missing password/i.test(message))
		return t("missingPassword");
	if (/Missing source id/i.test(message))
		return t("missingSourceId");
	if (/Config failed/i.test(message))
		return `${t("configFailed")}\n${message}`;
	if (/Login failed/i.test(message))
		return t("loginFailed");
	if (/WHEP response/i.test(message))
		return t("invalidWhep");
	if (/HTTP 401|HTTP 403/i.test(message))
		return t("authFailed");
	if (/HTTP 404/i.test(message))
		return t("apiMissing");
	if (/HTTP 502/i.test(message))
		return t("deviceUnreachable");
...
async function loadConfig()
{
	...
	const response = await fetch(`/config/${encodeURIComponent(sourceId)}`, {cache: "no-store"});
	if (!response.ok)
		throw new Error(`Config failed: HTTP ${response.status}`);
	const loaded = await response.json();
	messages = {...messages, ...(loaded.messages || {})};
	return loaded;
}
...
async function login()
{
	...
	const payload =
		await linkrFetch("/api/account/login", {method: "POST", json: {username: config.username, passwd}});
...
async function startPlayback()
{
	...
	const pc = new RTCPeerConnection({sdpSemantics: "unified-plan", bundlePolicy: "max-bundle"});
	...
	pc.createDataChannel("chat", {ordered: false, maxRetransmits: 0, priority: "high", protocol: "binary"});
	...
	try {
		...
		const response = await linkrFetch("/api/rtc/whep", {
			...
			json: {offer: pc.localDescription?.sdp || offer.sdp}
		});
...
function startHealthTimer(token)
{
	...
	healthTimer = setInterval(() => {
		...
		linkrFetch("/api/rtc/health", {
			method: "POST",
			headers: {authorization: `Bearer ${token}`},
			json: {stream_id: Number(streamId)}
		}).catch(() => {});
	}, 60000);
}
